...
| `FMHA_SM120_QSTAT_GRADS` | `bf16` | **Experimental — not production-qualified**: `fp8` runs the backward gradient MMAs in e4m3 (~12% faster step) but produced NaNs at ~1k steps in a real training run (per-row e4m3 amax is fragile under gradient outliers). Keep `bf16` for training; any fp8-grads adoption requires a >=2k-step loss A/B at your exact config. |
